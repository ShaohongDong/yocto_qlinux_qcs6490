...
std::vector<byte_track::BYTETracker::STrackPtr> byte_track::BYTETracker::update(const std::vector<Object>& objects)
{
    ...
    for (const auto &object : objects)
    {
        ...
        strack->detection_index = detection_index++;
        ...
        if (fuse_score_)
            for (size_t i = 0; i < dists.size(); ++i)
                for (size_t j = 0; j < dists[i].size(); ++j)
                    dists[i][j] = 1 - (1 - dists[i][j]) * det_stracks[j]->getScore();
...

    {
        ...
        if (fuse_score_)
            for (size_t i = 0; i < dists.size(); ++i)
                for (size_t j = 0; j < dists[i].size(); ++j)
                    dists[i][j] = 1 - (1 - dists[i][j]) * remain_det_stracks[j]->getScore();
    ...
    lost_stracks_ = subStracks(jointStracks(subStracks(lost_stracks_, tracked_stracks_), current_lost_stracks), jointStracks(removed_stracks_, current_removed_stracks));
    removed_stracks_ = current_removed_stracks; // Bound retained history.
