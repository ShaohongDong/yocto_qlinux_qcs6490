...
class Video {
    ...
    static GQuark index_key() {return g_quark_from_static_string("human-tracker-source-index");}
public:
        ...
        while(!state.stop && !interrupted) {
            ...
            if(!batch.empty()) {
                ...
                if(status==Video::Read::End) {
                    ...
                    decoded_previous+=video->decoded_count();video=std::make_unique<Video>(opt.input,opt.gui&&!opt.exact);tracker=std::make_unique<ai::Tracker>();++loop;loop_processed=0;continue;
                }
            } else {
            ...
            if(tracks.is_open()) {
                tracks<<std::setprecision(9)<<"{\"frame\":"<<count<<",\"source_frame\":"<<frame.source_index
                      ...
                      <<",\"pipeline_ms\":"<<frame.pipeline_ms<<",\"persons\":"<<boxes_json(frame.boxes)<<"}\n";
...
void draw_frame(cairo_t* cr,const Frame& frame,double width,double height,float threshold=.1f) {
    ...
    for(auto b:frame.boxes) {
        ...
        cairo_set_source_rgb(cr,color[0],color[1],color[2]);cairo_set_line_width(cr,2/scale);cairo_rectangle(cr,b.x1,b.y1,b.x2-b.x1,b.y2-b.y1);cairo_stroke(cr);
        static constexpr int edges[][2]={{0,1},{0,2},{1,3},{2,4},{5,6},{5,7},{7,9},{6,8},{8,10},{5,11},{6,12},{11,12},{11,13},{13,15},{12,14},{14,16}};
        for(const auto& e:edges) {
            const auto& a=b.keypoints[e[0]];const auto& z=b.keypoints[e[1]];
            if(a.confidence>=.3f&&z.confidence>=.3f) {cairo_move_to(cr,a.x,a.y);cairo_line_to(cr,z.x,z.y);cairo_stroke(cr);}
        }
        for(const auto& p:b.keypoints) if(p.confidence>=.3f) {cairo_arc(cr,p.x,p.y,3/scale,0,6.283185307);cairo_fill(cr);}
...
struct Ui {
    ...
    void begin() {
        ...
        {std::lock_guard<std::mutex> lock(state.mutex);state.latest.reset();state.error.clear();state.profile="[]";state.frames=0;state.latencies.clear();state.pipeline_latencies.clear();state.decoded=0;state.dropped=0;state.seconds=0;state.load_ms=0;state.input_side=0;state.model_sha256.clear();}
    ...
    void create() {
        window=gtk_window_new(GTK_WINDOW_TOPLEVEL);gtk_window_set_title(GTK_WINDOW(window),"Q6A Human Motion Tracking");gtk_window_set_default_size(GTK_WINDOW(window),960,540);
        auto* header=gtk_header_bar_new();gtk_header_bar_set_title(GTK_HEADER_BAR(header),"Human Tracker");gtk_header_bar_set_show_close_button(GTK_HEADER_BAR(header),TRUE);gtk_window_set_titlebar(GTK_WINDOW(window),header);
        ...
        auto* title=gtk_label_new("Human Motion Tracking");gtk_widget_set_halign(title,GTK_ALIGN_START);gtk_style_context_add_class(gtk_widget_get_style_context(title),"heading");gtk_box_pack_start(GTK_BOX(root),title,FALSE,FALSE,0);
        auto* subtitle=gtk_label_new("17 body keypoints  ·  Person IDs  ·  Motion trails");gtk_widget_set_halign(subtitle,GTK_ALIGN_START);gtk_style_context_add_class(gtk_widget_get_style_context(subtitle),"muted");gtk_box_pack_start(GTK_BOX(root),subtitle,FALSE,FALSE,0);
        ...
        g_signal_connect(demo,"clicked",G_CALLBACK(+[](GtkButton*,gpointer p){auto& u=*static_cast<Ui*>(p);u.opt.input=(std::filesystem::path(u.opt.model).parent_path().parent_path()/"data/sample.png").string();u.begin();}),this);
        ...
        g_timeout_add(100,+[](gpointer p)->gboolean {
            ...
            {std::lock_guard<std::mutex> lock(u.state.mutex);
                if(!u.state.error.empty()) gtk_label_set_text(GTK_LABEL(u.status),("Error: "+u.state.error).c_str());
                    ...
                    std::ostringstream text;text<<"NPU  ·  "<<u.state.latest->boxes.size()<<" people  ·  "<<std::fixed<<std::setprecision(1)<<u.state.latest->inference_ms<<" ms  ·  "<<(u.state.seconds>0?u.state.frames/u.state.seconds:0)<<" FPS  ·  "<<u.state.frames<<" frames  ·  "<<u.state.dropped<<" dropped"<<(u.state.done?"  ·  Finished":"");
...
int main(int argc,char** argv) {
    ...
    try {
        ...
        opt.model=(executable.parent_path().parent_path()/"share/human-tracker/models/pose.bin").string();
        for(int i=1;i<argc;++i) {
            std::string key=argv[i];
            if(key=="--self-test") {self_test();return 0;}
            if(key=="--exact") {opt.exact=true;continue;}
            if(key=="--gui") {opt.gui=true;continue;}
            if(key=="--help") {std::cout<<"human-tracker [--gui] --input FILE [--model CONTEXT.bin] [--report JSON] [--tracks JSONL] [--detections JSONL] [--batch LIST] [--exact] [--snapshot PNG] [--dump-output RAW] [--confidence 0.25] [--duration SECONDS] [--repeat COUNT]\n";return 0;}
    ...
    } catch(const std::exception& e) {std::cerr<<"human-tracker: "<<e.what()<<"\n";return 1;}
}
