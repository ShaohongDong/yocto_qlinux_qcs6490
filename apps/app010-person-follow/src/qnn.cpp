...
struct Qnn::Impl {
    ...
    void load(const std::string& path) {
        ...
        if(FIELD(input,rank)!=4 || FIELD(input,dimensions)[0]!=1 || FIELD(input,dimensions)[3]!=3)
            throw std::runtime_error("Expected NHWC RGB input with batch size 1");
        side=int(FIELD(input,dimensions)[1]);
        if((side!=416 && side!=640) || FIELD(input,dimensions)[2]!=uint32_t(side) || elements(input)!=size_t(side)*side*3)
            throw std::runtime_error("Expected square pose input with side 416 or 640");
        if(elements(output)!=channels*anchor_count(side) || FIELD(output,rank)!=3 || FIELD(output,dimensions)[0]!=1)
            throw std::runtime_error("Unexpected YOLO output");
        last=FIELD(output,dimensions)[2]==56;
        if(!last && FIELD(output,dimensions)[1]!=56) throw std::runtime_error("Unexpected output layout");
...
Qnn::~Qnn()=default;
int Qnn::input_side() const {return impl->side;}
