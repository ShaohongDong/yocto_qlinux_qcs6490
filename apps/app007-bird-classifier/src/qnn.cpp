...
struct Qnn::Impl {
    ...
    void load(const std::string& path) {
        ...
        if(n!=1 || !graphs) throw std::runtime_error("Expected one classifier graph");
        std::string name;
        auto capture=[&](const auto& g) {
            if(g.numGraphInputs!=1 || g.numGraphOutputs!=1) throw std::runtime_error("Expected one input and one classifier output");
        ...
        if(elements(input)!=224*224*3 || FIELD(input,rank)!=4 || FIELD(input,dimensions)[0]!=1 || FIELD(input,dimensions)[1]!=224 || FIELD(input,dimensions)[2]!=224 || FIELD(input,dimensions)[3]!=3)
            throw std::runtime_error("Expected NHWC input [1,224,224,3]");
        if(elements(output)!=20 || FIELD(output,rank)!=2 || FIELD(output,dimensions)[0]!=1 || FIELD(output,dimensions)[1]!=20)
            throw std::runtime_error("Expected classifier output [1,20]");
